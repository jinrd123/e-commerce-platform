...
## 59.利用路由独享守卫与组件内守卫增加登录后路由转跳的限制

登录后路由转跳的限制逻辑是："/trade"路由必须右"/shopcart"路由转跳而来（例如不能从"/home"路由转跳至"/trade"），"/pay"路由必须由"/trade"路由转跳而来，"/paysuccess"必须由"/pay"路由转跳而来。

在路由配置文件`@/router/index.js`中给单独的路由配置项增加属性`beforeEnter`（路由独享守卫）：
...
"/paysuccess"路由限制（"/paysuccess"必须由"/pay"路由转跳而来）我们通过组件内守卫完成：
...
面试时有必要看官网文档把全局路由守卫（全局前置、全局解析、全局后置）、独享路由守卫（`beforeEnter`配置项）、组件内路由守卫（`beforeRouterEnter`、`beforeRouterUpdate`、`beforeRouterLeave`写在组件内，与data配置项平级的方法）相关知识掌握，比如三个组件内守卫的不同特性：`beforeRouterEnter`中不能访问`this`、组件内路由导航`beforeRouterUpdate`的执行时机（路由组件不变，但路由参数改变时执行）还有**路由导航解析流程**（各种路由守卫的执行顺序）重点掌握。

## 60.使用插件实现图片懒加载以及自定义Vue自定义插件

所谓图片懒加载，就是说服务器数据到来之前，`<img :src="服务器返回的url">` 我们呈现别的图片。
...
~~~js
//引入一张图片，供vue-lazy插件使用
import lazyPicture from "@/assets/1.gif";
...
Vue.use(VueLazyload,{
    //设置加载默认的图片
    /*
    	通过阅读下面自定义插件相关的知识，我们了解到：
    	不管VueLazyload的实现逻辑是什么，现在这里的这个配置对象其实就是传给插件对象的install函数的第二个参数
    	然后VueLazyload实现的时候获取到loading属性，而且因为VueLazyload的使用方式是给标签用v-lazy，所以推测
    	实现的时候使用了Vue.directive("lazy",...)
    */
    loading: lazyPicture,
})
~~~

* 组件中使用插件（用`v-lazy=""`代替`v-bind:src=""`）
...
/*
	插件对象的install属性为一个方法
	一旦Vue.use()注册插件时会自动调用install方法
	@/main.js:
		import myPlugin from "@/plugins/myPlugin.js";
		Vue.use(myPlugin);//输出"注册插件时被调用"
*/
myPlugin.install = function() {
    console.log("注册插件时被调用");
}
...
以上是定义与使用自定义插件的基本流程，下面是自定义插件的`install`方法的参数
...
* Vue实例对象，可以通过`Vue.`使用一些方法（所有Vue属性都可以访问）
* `options`配置对象，Vue注册插件时传入

`@/main.js`：

~~~js
/*
	Vue.use的第二个参数传入的对象其实就是传给了自定义插件的		
	install函数的第二个参数
*/
Vue.use(myPlugin,{
    name:'upper'
...
例如：自定义一个插件myPlugin实现内部文本小写变成大写
...
myPlugin.install = function(Vue, options) {
    /*
    	Vue.directive(name, 标签使用回调函数)方法相当于自定义标签v-name
    	在html结构中可以使用v-name
    */
    Vue.directive(options.name, (element,params)=>{
        /*
        	element为绑定v-name的dom元素
        	params.value可以访问v-name指定的值
        */
...
三级联动导航鼠标离开某个导航项的事件用事件委派

## 2.节流优化

三级联动导航鼠标进入某个导航项的回调进行节流，利用lodash库的throttle函数

## 3.三级联动导航路由转跳

使用<a>标签代替<router-link>（消耗大），转跳回调的绑定用事件委派，但需要结合vue的自定义属性区别点击的是不是<a>标签，以及区别点击的是哪一级的<a>，通过`event.target.dataset`可以访问自定义属性。

## 4.给三级联动出现添加vue动画

<transition>标签包含`v-if/show`的结构，就有对应的vue内置css类设置动画：
...
## 5.nextTick激活（实例化swiper）swiper轮播图

激活时机：监视轮播图的数据，并在watch回调中的`this.nextTick`中执行`new Swiper`。因为轮播图数据更新之后（组件初始化时异步获取轮播图数据，在模板更新之后轮播图数据才来）执行watch回调，此时并没有更新模板，因为一开始轮播图数据为空，相关的dom结构并没有遍历出来，所以激活无效，所以为了等结构存在，即实例化放在`this.nextTick`中执行。

## 6.search组件发请求获取信息的时机

请求的所有参数都和路由传参相关，监听$route（用`Object.assign`构造参数）
...
Vue实例构建的某个生命周期中在Vue原型挂载$bus（把自己挂上去）
...
search页面删除keyword对应的面包屑之后删除头部组件中输入框中的输入内容
...
封装逻辑就是中间连续部分稳定存在，两边按钮根据计算出来的分页器的起始和中止位置设置显示和隐藏。

父组件除了传给分页器以上数据之外，还要监听分页器的改变页号的自定义事件（自定义事件实现子传父通信），父组件当前页号重新发送请求获取服务器数据

## 9.发请求进行增删改相关操作

`actions`中的方法定义为`async`函数，根据请求结果返回promise，组件中dispatch函数的返回值就是actions中方法的返回值，所以可以用`await`获取请求是否成功或者失败。组件中使用`try-catch`结构，await dispatch放在try中，后面根请求成功的逻辑，catch中写请求失败的逻辑。

## 10.利用uuid模拟用户唯一标识

uuid生成唯一字符串，detail仓库中存放，请求拦截器中添加请求头携带uuid，这样获取购物车数据实现后端存储。
...
## 12.vue监听修饰符`.prevent`清除dom的默认行为

在<form>中的<button>会有默认行为：页面转跳，所以我们绑定`click`事件的时候需要清除默认行为：`@click.prevent=""`

## 13.用户的登陆与注册

注册共两个请求：第一个请求通知服务器发送二维码给用户，第二个请求通知服务器进行注册，（与前端基本没关系）

登录：登录请求成功之后就把服务器返回的token存储至本地，然后请求拦截器中给请求头增加token字段
...
根据订单号发请求，服务器返回一个codeURL，也就是二维码的base64字符串的加密格式，我们借助qrcode可以将其转为base64字符串，利用elementUI的弹窗弹出这个字符串生成的<img>即可，弹窗打开过程中需要开启一个定时器一直发请求侦测支付状态。

## 16.独享路由守卫使用

使用场景：一些路由组件必须由另一个明确的路由组件转跳而来，不能从其他路由组件转跳而来，这时候就可以给要进入的路由配置独享路由守卫（独享路由守卫只有前置）：

trade组件必须由shopcart组件而来，trade组件的独享路由守卫：
